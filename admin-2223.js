...
console.info("De Stampertjes Developer Portal BUILD 2223 geladen");
  ...
  playerList.querySelectorAll("[data-delete-player]").forEach(btn=>{
    btn.addEventListener("click",async e=>{
      ...
      try{
        ...
        console.error(err);
        alert("Verwijderen is mislukt. Controleer of de v2.22.3 SQL-migratie is uitgevoerd.");
        btn.disabled=false;
...
function renderLevels(list){
  ...
  }).join(""):"<div class='small emptyBox'>Nog geen level-events geregistreerd. Speel v2.22.3 om deze data te vullen.</div>";
}
...
function renderEvents(list){
  ...
  `).join(""):"<div class='small emptyBox'>Nog geen v2.22.3-events.</div>";
}
...
async function refreshNewDashboard(){
  ...
  try{
    ...
    const podium=Array.isArray(hall?.podium)?hall.podium:[];
    const rec=$("recordsDashboard");
    if(rec)rec.innerHTML=podium.length?podium.map((x,i)=>`
      <div class="recordAdminRow"><span>${["🥇","🥈","🥉"][i]||`${i+1}.`}</span>
      <b>${esc(x.player_name||"SPELER")}</b><strong>${fmtDash(x.value)}</strong></div>`).join("")
      :"Nog geen records.";
...
(async()=>{
  const raw=window.STAMPERTJES_CONFIG?.version||"2.22.3";
...
async function loadV222Analytics(){
  ...
  }catch(e){el.textContent="v2.22.3 analytics nog niet beschikbaar — controleer SQL 007.";}
}
loadV222Analytics();

document.documentElement.dataset.adminBuild="2223";

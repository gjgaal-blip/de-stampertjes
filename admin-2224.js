...
console.info("De Stampertjes Developer Portal BUILD 2224 geladen");
  ...
  playerList.querySelectorAll("[data-delete-player]").forEach(btn=>{
    btn.addEventListener("click",async e=>{
      ...
      try{
        ...
        console.error(err);
        alert("Verwijderen is mislukt. Controleer of de v2.22.4 SQL-migratie is uitgevoerd.");
        btn.disabled=false;
...
function renderLevels(list){
  ...
  }).join(""):"<div class='small emptyBox'>Nog geen level-events geregistreerd. Speel v2.22.4 om deze data te vullen.</div>";
}
...
function renderEvents(list){
  ...
  `).join(""):"<div class='small emptyBox'>Nog geen v2.22.4-events.</div>";
}
...
(async()=>{
  const raw=window.STAMPERTJES_CONFIG?.version||"2.22.4";
...
async function loadV222Analytics(){
  ...
  }catch(e){el.textContent="v2.22.4 analytics nog niet beschikbaar — controleer SQL 007.";}
}
loadV222Analytics();

document.documentElement.dataset.adminBuild="2224";

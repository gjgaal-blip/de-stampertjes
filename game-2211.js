...
async function loadHallOfFame(){
  ...
  try{
    const [scoreResponse,statsResponse]=await Promise.all([
      fetch(`${SUPABASE_URL}/rest/v1/highscores?select=name,score,level,created_at&order=score.desc&limit=50`,{
    ...
    const rawScores=await scoreResponse.json();
    const stats=await statsResponse.json();
    const totals=stats?.totals||{};

    // One podium place per player name: duplicate score submissions do not occupy multiple podium positions.
    const seen=new Set();
    const podium=(Array.isArray(rawScores)?rawScores:[]).filter(s=>{
      const key=String(s.name||"SPELER").trim().toUpperCase();
      if(seen.has(key))return false;
      seen.add(key); return true;
    }).slice(0,3);
    const setPodium=(entry,nameEl,scoreEl)=>{
      nameEl.textContent=entry?String(entry.name||"SPELER").toUpperCase():"NOG VRIJ";
      scoreEl.textContent=entry?`${Number(entry.score)||0} punten · Lv${Number(entry.level)||1}`:"—";
    };
    ...
    hallApples.textContent=(Number(totals.apples_defeated)||0).toLocaleString("nl-NL");
    hallGames.textContent=(Number(totals.games_played)||0).toLocaleString("nl-NL");
    hallTeddyFinders.textContent=String(Number(totals.teddy_finders)||0);
    hallTeddyBadge.textContent=`${Number(totals.teddy_finders)||0} ONTDEKKER${Number(totals.teddy_finders)===1?"":"S"}`;
    ...
    [hallChampion,hallSecond,hallThird].forEach(el=>el.textContent="NIET BEREIKBAAR");
    [hallChampionScore,hallSecondScore,hallThirdScore,hallHighestLevel,hallApples,hallGames,hallTeddyFinders,hallTeddyBadge].forEach(el=>el.textContent="—");
